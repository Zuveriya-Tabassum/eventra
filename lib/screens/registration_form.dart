    ...
    try {
      ...
      if (mode == 'online' && paymentScreenshotBytes != null) {
        final ref = FirebaseStorage.instance
            .ref()
            .child('hackathon_payments/${user.uid}_${widget.eventName}.png');
        await ref.putData(
      ...
      if (mode == 'online') {
        ...
        await FirebaseFirestore.instance.runTransaction((trx) async {
          ...
          final oldHackathons = (data['hackathon'] ?? 0) as int;

          trx.set(
            participationRef,
            {
              'date': todayStr,
              'hackathon': oldHackathons + 1,
              'workshop': data['workshop'] ?? 0,
              'quiz': data['quiz'] ?? 0,
            },
    ...
    return Theme(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Hackathon Enrollment'),
        ),
                      ...
                      child: Column(
                        children: [
                          if (widget.qrUrl.isNotEmpty)
                            ...
                            QrImageView(
                              data:
                              'upi://pay?pa=upi@eventra&pn=EventraHack&am=$fee',
                              size: 180,
                            ),
                          const SizedBox(height: 10),
                          ElevatedButton.icon(
