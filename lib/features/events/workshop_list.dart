...
class _WorkshopListPageState extends State<WorkshopListPage>
  ...
  void initState() {
    ...
    staticWorkshops = [
      Workshop(
        ...
        description: 'Intensive mobile app development workshop.',
        ...
        offlineContactEmail: 'gdsc@clg.edu',
      ),
    ];
  }
  ...
  List<Workshop> _filterByTab(List<Workshop> all, int tabIndex) {
    if (tabIndex == 0) return all.where((w) => w.status == 'upcoming').toList();
    if (tabIndex == 1) return all.where((w) => w.status == 'ongoing').toList();
    return all.where((w) => w.status == 'completed').toList();
  }
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snap) {
          ...
          return TabBarView(
            controller: _tabController,
            children: [
              _buildWorkshopList(_filterByTab(all, 0), user?.uid),
              _buildWorkshopList(_filterByTab(all, 1), user?.uid),
              _buildWorkshopList(_filterByTab(all, 2), user?.uid),
            ],
          );
        },
      ),
  ...
  Widget _buildWorkshopList(List<Workshop> list, String? uid) {
    return ListView(
...
class _WorkshopCardState extends State<WorkshopCard> {
  ...
  bool get _canEdit =>
      ...
      (widget.role == 'Club Head' &&
          widget.workshop.createdBy == widget.currentUserId);
  ...
  Future<void> _edit() async {
    ...
    if (widget.fixedHalls.contains(w.venue)) {
      selectedHall = w.venue;
    } else if (w.venue.toLowerCase().startsWith('room ')) {
      selectedHall = 'Room no';
      roomNoC.text = w.venue.substring(5);
    } else {
      selectedHall = 'Other hall';
      otherHallC.text = w.venue;
    }

    DateTime deadline = w.deadline;
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(
          '${DateFormat('MMM dd').format(w.deadline)} • ${w.venue}',
        ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (w.qrUrl.isNotEmpty) Image.network(w.qrUrl, height: 120),
                const SizedBox(height: 8),
                Text(
                ...
                if (w.createdByName != null)
                  Text(
                    'Created by: ${w.createdByName}',
                    style: const TextStyle(fontSize: 10, color: Colors.grey),
                  ),
                if (w.updatedByName != null)
                  Text(
                    'Last edited by: ${w.updatedByName}',
                    style: const TextStyle(fontSize: 10, color: Colors.grey),
                  ),
                Row(
                  ...
                  children: [
                    ...
                    if (widget.role == 'Participant' && w.status == 'upcoming')
                      ElevatedButton(
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => RegistrationFormPage(
                                ...
                                fee: w.fee,
                                qrUrl: w.qrUrl,
                                offlineContactPhone: w.offlineContactPhone,
                                offlineContactEmail: w.offlineContactEmail,
                              ),
                            ),
                          );
                        },
...
class _AddWorkshopCard extends StatefulWidget {
  final List<String> fixedHalls;
  ...
  @override
  State<_AddWorkshopCard> createState() => _AddWorkshopCardState();
}

class _AddWorkshopCardState extends State<_AddWorkshopCard> {
  @override
  Widget build(BuildContext context) {
    return Card(
      elevation: 2,
